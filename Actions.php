<?php
require_once('DBConnection.php'); //Embed php code from another file DBconnection.php
Class Actions extends DBConnection{
    function __construct(){
        parent::__construct();   //Prent Allow Access to Inherited class
    }
    function __destruct(){
        parent::__destruct();
    }
    function login(){
        extract($_POST);      //extract() function imports variables into the local symbol table from an array
        $sql = "SELECT * FROM admin_list where username = '{$username}' and `password` = '".md5($password)."' ";  //MD5 hash is created by taking a string of an any length and encoding it into a 128-bit fingerprint
        @$qry = $this->query($sql)->fetchArray();    //$this is a reserved keyword in PHP that refers to the calling object.
        if(!$qry){
            $resp['status'] = "failed";
            ...
            foreach($qry as $k => $v){      //is_numeric () function checks whether a variable is a number or a numeric string
                if(!is_numeric($k))
                $_SESSION[$k] = $v;       //The $_SESSION is a global constant in PHP. We can set and access the session data through the $_SESSION variable.
            }
        }
        return json_encode($resp);    //json_encode() function is an inbuilt function in PHP which is used to convert PHP array or object into JSON representation.
    }
    function employee_login(){
        extract($_POST);
        $sql = "SELECT * FROM employee_list where email = '{$email}' and `password` = '".md5($password)."' ";
        @$qry = $this->query($sql)->fetchArray();    //fetch_array () / mysqli_fetch_array () function fetches a result row as an associative array, a numeric array, or both.
        if(!$qry){
            $resp['status'] = "failed";
    ...
    function employee_logout(){
        session_destroy();
        header("location:./");      //./ usually denotes the current folder that your program or script is in
    }
