                ...
                <tr>
                    ...
                    <th class="text-center py-0 px-1">
                        <div class="btn-group" role="group">
                            ...
                            <li><a class="dropdown-item update_status" data-id = '<?php echo $row['user_id'] ?>' href="javascript:void(0)" data-name = '<?php echo $row['fullname'] ?>' data-status = '<?php echo $row['status'] ?>'>Update Status</a></li>
                            <li><a class="dropdown-item view_data" data-id = '<?php echo $row['user_id'] ?>' href="javascript:void(0)" data-name = '<?php echo $row['fullname'] ?>' >View</a></li>
                            <li><a class="dropdown-item edit_data" data-id = '<?php echo $row['user_id'] ?>' href="javascript:void(0)" data-name = '<?php echo $row['fullname'] ?>' >Edit</a></li>
...
<script>
    $(function(){
        $('#create_new').click(function(){
            uni_modal('Add New user',"manage_user.php","mid-large")
        })
        $('.update_status').click(function(){
            uni_modal('Update '+$(this).attr('data-name')+'\'s Status',"update_user_status.php?id="+$(this).attr('data-id')+"&status="+$(this).attr('data-status'))
        })
        $('.view_data').click(function(){
            uni_modal('View User Details',"view_user.php?id="+$(this).attr('data-id'),"mid-large")
        })
        $('.edit_data').click(function(){
            uni_modal('Edit User Details',"manage_user.php?id="+$(this).attr('data-id'),"mid-large")
        })
