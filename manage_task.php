                ...
                <div class="col-md-4">
                    ...
                    <label for="assign_to" class="control-label">Assign To</label>
                        <select type="assign_to" name="assign_to[]" id="assign_to" multiple required class="form-select form-select-sm rounded-0 select2" data-placeholder = "Select Employee Here" >
                            <?php
                            $employee_qry= $conn->query("SELECT * FROM `employee_list`, `department_list` where employee_list.department_id=department_list.department_id and  employee_id !='{$_SESSION['employee_id']}' ".(count($assignees) > 0 ? " OR ( employee_id in (".(implode(',',$assignees))."))" : "")."  order by `fullname` asc");
                            while($row = $employee_qry->fetchArray()):
                            ?>
                            <option value="<?php echo $row['employee_id'] ?>" <?php echo in_array($row['employee_id'],$assignees) ? 'selected' : '' ?>><?php echo $row['name'].'-'.$row['fullname'] ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                </div>
